    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-950 via-blue-950 to-black py-8 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto">
        ...
        >
          ...
          <div className="flex items-center gap-3 mb-2">
            <div className="relative">
              ...
              <div className="absolute inset-0 w-10 h-10 text-blue-500 animate-ping opacity-20">
        ...
        <div className="grid grid-cols-2 md:grid-cols-6 gap-4 mb-8">
          {[
            { label: 'Total', value: stats.total, color: 'from-blue-500 to-cyan-500', icon: Activity },
            { label: 'Critical', value: stats.critical, color: 'from-red-500 to-red-600', icon: AlertCircle },
            { label: 'High', value: stats.high, color: 'from-orange-500 to-orange-600', icon: AlertTriangle },
            { label: 'Medium', value: stats.medium, color: 'from-yellow-500 to-yellow-600', icon: Activity },
            { label: 'Low', value: stats.low, color: 'from-green-500 to-green-600', icon: Shield },
            { label: 'Active', value: stats.active, color: 'from-purple-500 to-purple-600', icon: TrendingUp }
          ].map((stat, index) => (
            <motion.div
              key={index}
              ...
              className="bg-gradient-to-br from-gray-900/50 to-gray-800/30 backdrop-blur-xl border border-gray-700/50 rounded-xl p-4 hover:border-blue-500/50 transition-all"
            >
            ...
            <div className="space-y-4">
              {threats.map((threat, index) => (
                <motion.div
                  ...
                  exit={{ opacity: 0, x: 20 }}
                  transition={{ delay: index * 0.05 }}
                  className="bg-gradient-to-br from-gray-900/50 to-gray-800/30 backdrop-blur-xl border border-gray-700/50 rounded-xl p-6 hover:border-blue-500/50 transition-all"
                >
                        ...
                        {threat.indicators && threat.indicators.length > 0 && (
                          <div className="mt-3 flex flex-wrap gap-2">
                            {threat.indicators.map((indicator, i) => (
                              <span
                                key={i}
                                className="px-2 py-1 bg-red-500/10 border border-red-500/30 rounded text-xs text-red-400"
                              >
                                {indicator}
                              </span>
                            ))}
